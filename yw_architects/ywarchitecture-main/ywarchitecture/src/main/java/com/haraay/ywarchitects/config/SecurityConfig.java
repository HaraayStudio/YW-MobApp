...
public class SecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		http.addFilterBefore(corsFilter, SessionManagementFilter.class).csrf(
				csrf -> csrf.disable()).authorizeHttpRequests(
						auth -> auth

								// ── Public endpoints ──────────────────────────────────────
								.requestMatchers("/api/auth/**").permitAll().requestMatchers("/ws-notifications/**")
								.permitAll().requestMatchers("/api/website/projects").permitAll()
								.requestMatchers("/api/website/getprojectbyid/**").permitAll()
								.requestMatchers("/api/website/**").hasAnyRole(ADMIN, CO_FOUNDER, SR_ARCHITECT,HR)
								.requestMatchers("/api/rera/**")
								.hasAnyRole(ADMIN, CO_FOUNDER, SR_ARCHITECT, JR_ARCHITECT, SR_ENGINEER, DRAFTSMAN,
										LIAISON_MGR, LIAISON_OFF, LIAISON_ASST, HR)
								...
								.hasAnyRole(ADMIN, CO_FOUNDER, SR_ARCHITECT, JR_ARCHITECT, SR_ENGINEER, DRAFTSMAN,
										LIAISON_MGR, LIAISON_OFF, LIAISON_ASST, HR, CLIENT)
								...
								.hasAnyRole(ADMIN, CO_FOUNDER, SR_ARCHITECT, JR_ARCHITECT, SR_ENGINEER, DRAFTSMAN,
										LIAISON_MGR, LIAISON_OFF, LIAISON_ASST, HR)
								.requestMatchers("/api/attendance/recordmycheckIn")
								.hasAnyRole(ADMIN, CO_FOUNDER, SR_ARCHITECT, JR_ARCHITECT, SR_ENGINEER, DRAFTSMAN,
										LIAISON_MGR, LIAISON_OFF, LIAISON_ASST, HR)
								.requestMatchers("/api/attendance/recordmycheckout")
								.hasAnyRole(ADMIN, CO_FOUNDER, SR_ARCHITECT, JR_ARCHITECT, SR_ENGINEER, DRAFTSMAN,
										LIAISON_MGR, LIAISON_OFF, LIAISON_ASST, HR)
								.requestMatchers("/api/attendance/getmymonthlyattendance")
								.hasAnyRole(ADMIN, CO_FOUNDER, SR_ARCHITECT, JR_ARCHITECT, SR_ENGINEER, DRAFTSMAN,
										LIAISON_MGR, LIAISON_OFF, LIAISON_ASST, HR)
								.requestMatchers("/api/attendance/**").hasAnyRole(HR, ADMIN, CO_FOUNDER)
	...
	@Primary // ✅ marks this as the default for Spring Security's HttpSecurity
