...
class InvoiceReportView extends StatelessWidget {
        ...
        child: Center(
          child: Container(
            ...
            padding: EdgeInsets.all(
              MediaQuery.of(context).size.width < 600 ? 20 : 50,
            ),
            ...
            child: Column(
              ...
              children: [
                    ...
                    Expanded(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _buildHeaderInfo(
                            'Address :- Office No.313, Fortuna Business Hub,',
                          ),
                          _buildHeaderInfo('Near Shivar Chouk, Pimple Saudagar, Pune - 411018'),
                ...
                _buildTable([
                  ...
                  _buildTableRow([
                    'Total Net Amount',
                    '₹${(invoice['netAmount'] ?? 0.0).toStringAsFixed(2)}',
                  ]),
                  _buildTableRow([
                    'State Tax (SGST) 9%',
                    '₹${(invoice['sgstAmount'] ?? 0.0).toStringAsFixed(2)}',
                  ]),
                  _buildTableRow([
                    'Central Tax (CGST) 9%',
                    '₹${(invoice['cgstAmount'] ?? 0.0).toStringAsFixed(2)}',
                  ]),
                  _buildTableRow([
                    'GROSS TOTAL AMOUNT',
                    '₹${(invoice['grossAmount'] ?? 0.0).toStringAsFixed(2)}',
                  ], isFooter: true),
                ]),
                ...
                const Center(
                  child: Column(
                    children: [
                      ...
                      SizedBox(height: 4),
                      Text(
                        '020 40038445 | 9623901901 | yogeshrw24@gmail.com',
        ...
        children: [
          ...
          Flexible(
            child: Text(
              value,
              textAlign: TextAlign.right,
              style: const TextStyle(fontSize: 10, fontWeight: FontWeight.normal, color: Colors.black),
  ...
  Widget _buildTable(
    List<TableRow> rows, {
    Map<int, TableColumnWidth>? customFlex,
  }) {
    return Table(
      columnWidths:
          customFlex ?? const {0: FlexColumnWidth(2.5), 1: FlexColumnWidth(1)},
  ...
  Future<void> _printInvoice(BuildContext context) async {
    final pdfBytes = await InvoicePdfHelper.generateInvoice(
      invoice: invoice,
      isTax: isTax,
    );
  ...
  Future<void> _downloadInvoice(BuildContext context) async {
    ...
    final fileName =
        '${isTax ? "Tax" : "Proforma"}_Invoice_${invoice['invoiceNumber'] ?? invoice['id']}.pdf';
