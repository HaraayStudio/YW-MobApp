...
const _quotStatusCfg = {
  'DRAFT': {'bg': Color(0xFFF3F4F6), 'fg': Color(0xFF374151), 'dot': Color(0xFF9CA3AF), 'label': 'Draft'},
  'SENT': {'bg': Color(0xFFDBEAFE), 'fg': Color(0xFF1E40AF), 'dot': Color(0xFF3B82F6), 'label': 'Sent'},
  'ACCEPTED': {'bg': Color(0xFFDCFCE7), 'fg': Color(0xFF14532D), 'dot': Color(0xFF22C55E), 'label': 'Accepted'},
};
  ...
  void _applyFilters() {
    setState(() {
      _filteredInquiries = _allInquiries.where((item) {
        final matchesSearch =
            (item['personName'] ?? '').toString().toLowerCase().contains(_searchQuery.toLowerCase()) ||
      ...
      child: Column(
        children: [
          ...
          Icon(Icons.question_answer_outlined,
              size: 70, color: AppColors.outlineVariant),
...
class _InquiryDetailSheetState extends State<_InquiryDetailSheet>
  ...
  Future<void> _fetchQuotations() async {
    ...
    final data = await QuotationService.getQuotationsByPreSale(widget.item['srNumber']);
    if (mounted) {
      setState(() {
  ...
  Widget _buildDetailsTab(dynamic item) {
    ...
    if (dateStr.isNotEmpty) {
      try {
        formattedDate = DateFormat('d MMM yyyy').format(DateTime.parse(dateStr));
      } catch (_) {}
    }
            ...
            children: [
              ...
              TextButton.icon(
                ...
                style: TextButton.styleFrom(
                  foregroundColor:
                      _showAddForm ? AppColors.error : AppColors.primary,
                  textStyle:
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          Text('Click "+ Add Quotation" to create one',
              style: GoogleFonts.plusJakartaSans(
          ...
          Row(
            children: [
              ...
              if (!quotation.sended)
                ...
                _QuotMeta(Icons.check_rounded, 'Sent', color: const Color(0xFF1E40AF)),
              if (quotation.accepted) ...[
                const SizedBox(width: 16),
                _QuotMeta(Icons.check_circle_outlined, 'Accepted',
              ...
              if (!isAccepted)
                TextButton.icon(
                  ...
                  icon: const Icon(Icons.check_circle_outline_rounded,
                      size: 16, color: Color(0xFF14532D)),
                  label: Text('Accept',
                      style: GoogleFonts.plusJakartaSans(
                          fontWeight: FontWeight.w700,
                          color: const Color(0xFF14532D),
                  ...
                  style: TextButton.styleFrom(
                    padding:
                        const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                    backgroundColor:
                        const Color(0xFF14532D).withOpacity(0.06),
                    shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(8)),
                  ),
                ),
...
class _EditEnquirySheetState extends State<_EditEnquirySheet> {
  ...
  void initState() {
    ...
    _conclusionCtrl =
        TextEditingController(text: widget.item['conclusion'] ?? '');
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              value:
                  ['NEW', 'Onboarded', 'Not Onboarded'].contains(_status)
                      ? _status
                      : 'NEW',
              decoration: AppTheme.inputDecoration('Select status'),
              items: ['NEW', 'Onboarded', 'Not Onboarded']
                  .map((s) => DropdownMenuItem(value: s, child: Text(s)))
                  .toList(),
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            Text('Capture details for the new prospect',
                style: GoogleFonts.plusJakartaSans(
                    fontSize: 13, color: AppColors.onSurfaceVariant)),
            const SizedBox(height: 24),
            Row(
              children: [
                Expanded(
                    child: _ToggleBtn(
                        ...
                        onTap: () =>
                            setState(() => _isExistingClient = true))),
                const SizedBox(width: 12),
                Expanded(
                    child: _ToggleBtn(
                        label: 'New Client',
            ...
            if (_isExistingClient) ...[
                ...
                children: [
                  Expanded(
                      child: TextField(
                          ...
                          decoration: AppTheme.inputDecoration('Phone'))),
                  const SizedBox(width: 12),
                  Expanded(
                      child: TextField(
              ...
              TextField(
                  ...
                  decoration:
                      AppTheme.inputDecoration('Portal Password'),
                  obscureText: true),
            ],
            const Padding(
            ...
            TextField(
              ...
              maxLines: 3,
              decoration:
                  AppTheme.inputDecoration('Initial discussion notes...'),
            ),
            const SizedBox(height: 32),
            GoldGradientButton(
