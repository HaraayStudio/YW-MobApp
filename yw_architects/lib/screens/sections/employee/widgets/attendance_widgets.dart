    ...
    return FittedBox(
      ...
      child: Row(
        ...
        children: [
          ...
          if (absent > 0) const SizedBox(width: 4),
          if (absent > 0)
...
class _AttendanceDetailsPopupState extends State<AttendanceDetailsPopup> {
  ...
  Widget build(BuildContext context) {
    ...
    final presentCount = widget.records
        .where((r) => r['status']?.toString().toUpperCase() == 'PRESENT')
        .length;
    final lateCount = widget.records
      ...
      child: Column(
        children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        '${widget.records.length} employee records',
                        style: const TextStyle(
                          fontSize: 12,
                          color: AppColors.onSurfaceVariant,
          ...
          Container(
            ...
            child: const Row(
              children: [
                Expanded(flex: 8, child: FittedBox(fit: BoxFit.scaleDown, alignment: Alignment.centerLeft, child: Text('#', style: _hStyle))),
                Expanded(flex: 32, child: FittedBox(fit: BoxFit.scaleDown, alignment: Alignment.centerLeft, child: Text('EMPLOYEE', style: _hStyle))),
                Expanded(
                  flex: 20,
                  child: FittedBox(
                    fit: BoxFit.scaleDown,
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, i) {
                final r = filtered[i];
                final status = (r['status']?.toString() ?? 'UNMARKED')
                    .toUpperCase();
                return Container(
