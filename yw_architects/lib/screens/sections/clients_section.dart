...
class _Client {
  ...
  factory _Client.fromJson(Map<String, dynamic> json) {
    ...
    return _Client(
      ...
      gst: json['GSTCertificate']?.toString() ?? json['gstcertificate']?.toString() ?? '',
...
class _ClientsSectionState extends State<ClientsSection> {
  ...
  Future<void> _fetchClients() async {
    ...
    try {
      ...
      final parsed = rawData.map((e) => _Client.fromJson(e as Map<String, dynamic>)).toList();
      setState(() {
        _clients = parsed;
        _isLoading = false;
      });
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                const Text('Add Client', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700, color: AppColors.onSurface)),
                IconButton(
            ...
            _FormField(label: 'FULL NAME', hint: 'Anand Kapoor', controller: _nameCtrl),
            const SizedBox(height: 12),
            _FormField(label: 'EMAIL', hint: 'client@email.com', controller: _emailCtrl, keyboard: TextInputType.emailAddress),
            const SizedBox(height: 12),
            _FormField(label: 'PHONE', hint: '9876543210', controller: _phoneCtrl, keyboard: TextInputType.phone, inputFormatters: [FilteringTextInputFormatter.digitsOnly]),
            const SizedBox(height: 12),
            _FormField(label: 'ADDRESS', hint: 'City, State', controller: _addressCtrl),
            const SizedBox(height: 12),
            ...
            const SizedBox(height: 12),
            Row(children: [
              Expanded(child: _FormField(label: 'PAN', hint: 'ABCDE1234F', controller: _panCtrl, maxLength: 10, inputFormatters: [UpperCaseTextFormatter()])),
              const SizedBox(width: 12),
              Expanded(child: _FormField(label: 'GST NUMBER', hint: '22AAAAA0000A1Z5', controller: _gstCtrl, maxLength: 15, inputFormatters: [UpperCaseTextFormatter()])),
            ]),
            const SizedBox(height: 28),

            _isLoading
                ? const Center(child: CircularProgressIndicator(color: AppColors.primary))
                : GoldGradientButton(text: 'Add Client', icon: Icons.person_add_rounded, onTap: _submit),
...
class _ClientCard extends StatelessWidget {
  ...
  const _ClientCard({required this.client, required this.onTap, required this.onToast});
          ...
          children: [
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(client.name, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 15, color: AppColors.onSurface)),
                  const SizedBox(height: 4),
                  Row(
                     children: [
                        const Icon(Icons.email_rounded, size: 12, color: AppColors.onSurfaceVariant),
                        const SizedBox(width: 4),
                        Text(client.email.isNotEmpty ? client.email : 'No Email', style: const TextStyle(fontSize: 12, color: AppColors.onSurfaceVariant)),
                     ]
                  ),
                  const SizedBox(height: 2),
                  Row(
                     children: [
                        const Icon(Icons.phone_rounded, size: 12, color: AppColors.onSurfaceVariant),
                        const SizedBox(width: 4),
                        Text(client.phone.isNotEmpty ? client.phone : 'No Phone', style: const TextStyle(fontSize: 12, color: AppColors.onSurfaceVariant)),
                     ]
                  )
                ],
              ),
            ...
            Container(
               ...
               child: const Icon(Icons.chevron_right_rounded, color: AppColors.onSurfaceVariant, size: 20),
            )
          ],
        ),
      ),
...
class _SectionLabel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: const BoxDecoration(border: Border(left: BorderSide(color: AppColors.primaryContainer, width: 3))),
...
class _DropdownLabel extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Text(text, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: AppColors.outline, letterSpacing: 0.5));
  }
}
...
class _FormField extends StatelessWidget {
  ...
  const _FormField({required this.label, required this.hint, required this.controller, this.keyboard, this.maxLength, this.inputFormatters});
        ...
        TextField(
          ...
          decoration: InputDecoration(
            ...
            suffixIcon: IconButton(
              icon: Icon(visible ? Icons.visibility_off_rounded : Icons.visibility_rounded, color: AppColors.onSurfaceVariant, size: 20),
              onPressed: onToggle,
            ),
          ),
        ),
  ...
  Widget build(BuildContext context) {
    final child = Container(
      ...
      child: Row(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Icon(icon, color: gradient ? Colors.white : AppColors.onSurface, size: 16),
          const SizedBox(width: 8),
          Text(label, style: TextStyle(color: gradient ? Colors.white : AppColors.onSurface, fontSize: 13, fontWeight: FontWeight.w600)),
        ],
      ),
    );
      ...
      children: [
         ...
         Column(
            ...
            children: [
               ...
               Text(val, style: const TextStyle(fontSize: 13, color: AppColors.onSurface, fontWeight: FontWeight.w500)),
            ]
         )
      ]
   );
